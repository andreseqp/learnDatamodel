...
<!-- situations where cleaners have to choose will be rare if cleaner fidh relative  -->
...
the three model ("Full model", "chaining only" and "penalty only"). We show 
...
here the marginal posterior distributions, given that they do not bring 
biological insight. Their visualizations can be found in the supplementary 
...
#   stat_halfeye(aes(fill=stat(cut(x,breaks = cuts.3$scaleConst))),
#                point_interval = mode_hdi, .width = c(.66, .95),
#                point_size=3,
#                show.legend=FALSE) +
#   scale_fill_manual(values=c("gray85","skyblue","gray85"))+
...
```{r diagnosticsfull, fig.cap='(ref:diagnosticsfull)',echo=FALSE,fig.height=5}
...
```{r diaggam,fig.cap='(ref:diagnosticsGam)',echo=FALSE}
...
<!-- ```{r diag.neg,fig.cap='(ref:diag.neg)'} -->
