...
discovering interspecific variation in *higher* cognitive abilities; or 
...
through past events[@dally_Foodcaching_2006]. However, they often face challenges proposing alternative scenarios where simple associations can bring about the 
predicted behavioural outcome [@suddendorf_Evolution_2007]. Similarly, attempts 
to show *theory of mind* in non-human animals face objections 
justified by alternative mechanisms that bring about similar behavioural 
results [@heyes_Theory_1998]. Such controversies are usually settled by 
...
parrots solve this task as well [@pepperberg_Can_2014]. This interspecific
...
<!-- explanation for cleaner fish ability to prefer visitors is supported.  -->
...
distributions for a full model, including chai %>% ning ($\gamma$) and penalty 
...
```{r post, fig.cap='(ref:post)',out.width='100%',fig.height=6}
library(data.table)
library(coda)
library(ggplot2)
library(cowplot)
...
scen1<-"MCMCclean_gam_Nrew_sca_old_"
scen2<-"MCMCclean_gam_sca_old_"
scen3<-"MCMCclean_Nrew_sca_old_"
...
panel (e and f) from a model with only penalty.

```{r pred, fig.cap='(ref:pred)',out.width='100%',fig.height=5}
...
(ref:rawdata) Relation between the response variable used in this study and the 
...
initial round for all fish. 


```{r rawdata,fig.cap='(ref:rawdata)'}
plot_grid(initANDRev,init,ncol = 2)

```

(ref:scaConst) Posterior distributions for scaling constant for the three model 
...
```{r scaConst, fig.cap='(ref:scaConst)',fig.pos = 'H', out.extra = "",out.width='95%',fig.height=4}
...
```{r diagnosticsfull, fig.cap='(ref:diagnosticsfull)',message=FALSE,echo=FALSE,fig.height=5}
...
gelman.full<-ggplot(melt(gp.dat.frame, c("q","last.iter"), value.name="shrink_factor"), 
  ...
  geom_line() +
  facet_grid(rows = vars(variable), labeller= labeller(function(var,value){
  return(parlab[value])})) +
...
```{r diaggam,fig.cap='(ref:diagnosticsGam)',echo=FALSE,message=FALSE}
...
```{r diagNeg,fig.cap='(ref:diagNeg)',message=FALSE}
