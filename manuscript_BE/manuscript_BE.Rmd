...
#     #, Rue Emile-Argand 11, CH-2000, Neuchatel, Switzerland. +41 32 718 31 24
#   - name: Zegni Triki
#     affiliation: a
#     affiliation: b
#   - name: Redouan Bshary
...
lay_summary: |
  Cleaner fish eat ectoparasites off other fishes, so-called clients. It regularly happens that two clients seek a cleaner's service simulatenously. Cleaners benefit from prioritising clients unwilling to wait, so they can feed on those willing to wait. To make the right choice, cleaners must somehow “look” into the future to anticipate consequences of current choices. By combining a learning model with data, we show that cleaners estimate the long-term value of their actions rather than using simpler heuristics. Estimating long-term value is a mechanism involved in human foresight.
header-includes: |
  ```{=latex}
  \usepackage{float}
  \usepackage{graphicx}
...
the behaviour and the collor/pattern cue [@wismer_Cuebased_2019]. 
...
first_col<-plot_grid(nrow=3,align = "v",byrow = TRUE,
          cont.obs.pred.both,cont.obs.pred.gam,cont.obs.pred.Nrew,
          labels=c('a','c','e'),rel_heights = c(1.4,1,1))
second_col<-plot_grid(nrow=3,align = "v",byrow = TRUE,
          scatter.obs.pred.both,scatter.obs.pred.gam,scatter.obs.pred.Nrew,
          labels=c('b','d','f'))
plot_grid(first_col,second_col,rows = 1)
```
...
cleaner fish relative abundance is low (Fig. \ref{fig:pred} c). In contrast,
the models including penalty (with and without chaining) predict high 
preference for the visitor option (Fig. \ref{fig:pred} a and e).
...
low preference for the visitor (Fig. \ref{fig:pred}a,b,e). 
Note, however, we calculated 
preferences shown in figure \ref{fig:pred} left panels by using only the model 
